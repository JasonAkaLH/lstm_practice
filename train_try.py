...
if __name__ == '__main__':
    ...
    print(data)
    train_dt, valid_dt, test_dt, _, _, y_max, y_min = nn_seq_us(data, 64, 'High', features_size=50)
    print(train_dt.dataset.__len__())
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    model_path = './saved_model/fishing_path.pth'
    
    train(input_size=len(data.columns), hidden_size=64, num_layers=1, output_size=1, batch_size=64, lr=0.001,
          weight_decay=0.01, step_size=30, gamma=0.1, optimizer='Adam', epochs=3000, train_dt=train_dt, val_dt=valid_dt,
          path=model_path, compute_device=device, save_state=False)
